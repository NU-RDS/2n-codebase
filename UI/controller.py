...
class Controller:
    def __init__(self):
        # Set this to none because we want use only one serial port. 
        # Let the GUI to connect port.
        self.MSI = None # MotorSerialInterface(port='/dev/ttyACM0', baud_rate=115200, timeout=1)
        self.kp = 0.15
        self.ki = 0.0
        self.kd = 0.0
        self._r = 0.1
    ...
    def motor_to_joint(
        ...
        motor_positions_initial: np.ndarray,
        joint_angles_initial: np.ndarray,
        trans_mat: np.ndarray
    ) -> np.ndarray:
        """
        convert the motor position to the joint position.
        parameter:
            motor_positions (np.ndarray): current motor position当前电机位置（弧度），形状 (4,)
            motor_positions_initial (np.ndarray): last motor status(rad)，形状 (4,)
            joint_angles_initial (np.ndarray): (2,)
            trans_mat (np.ndarray): (2,4)
        
        返回:
            joint_position(np.ndarray): (2,)
        """
        delta_motor = motor_positions - motor_positions_initial
        return trans_mat @ delta_motor + joint_angles_initial

    def joint_to_motor_torques(
        joint_torques: np.ndarray, 
        trans_mat: np.ndarray
    ) -> np.ndarray:
        """
        Joint back to motor. 
        Maybe not useful. just put it here 
        """
        return trans_mat.T @ joint_torques  
